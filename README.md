...
- **What's working today**:
  - A dark-themed shell — a primary icon sidebar (Equipment, Star Atlas, Planning, Imaging, Guiding, Focus, Solve, Library, Science, Options — with Planning opening onto Targets, Sessions and Scheduler, and Science onto Variable Stars) plus a context-sensitive second panel per section, a top-bar Observatory/Pier selector, and a status bar.
  - **Planning > Sessions**: per-Pier, block-based session authoring — a right-hand drag-and-drop palette of action blocks (Target, Image, Filter Change, Cool/Warm Camera, Autofocus, Plate Solve, Guide Start/Stop, Dither, Flat Capture, Park/Unpark Mount, Meridian Flip, Dome Open/Close/Sync, Notification) dropped into one bordered card per session, with Save / Save as Template / Load from Template / Schedule / Delete controls; a scheduled session locks read-only with a red border. Selecting a target on the Star Atlas auto-creates a session pre-populated with a Target block. Session/scheduler-job data and named templates persist to the same shared database as everything else. Translating an authored session into real device I/O at run time isn't built yet.
  - **Planning > Scheduler**: the per-Pier job queue a Session's Schedule control submits to — priority reordering, an Edit dialog for constraints and startup/completion conditions, an altitude-trajectory chart per job, and Remove. A job that completes deletes its originating session; one withdrawn with Remove/Deschedule returns to an editable draft. The queue survives an application restart. Actually triggering capture from a runnable job isn't built yet.
  ...
  - **Framing Assistant**: opened contextually — never a top-level menu item — via the Imaging tab's **Framing…** control: a DSS survey image for the target with the FOV rectangle (or every pane's rectangle, for a mosaic grid) drawn on top, updating live as rotation/mosaic-grid/overlap settings change, cached offline after the first fetch. FOV is computed from the selected optical train; rotation is always available — with a rotator connected, accepting the dialog moves it to the requested angle, and with none connected the tilt is instead covered by an auto-sized mosaic of un-rotated panels (a status line says which applies), with the originally-requested tilted frame shown as a dashed outline for context. Defining a mosaic adopts it as the Imaging tab's active capture target — the tab shows a "Mosaic active" status line and its Capture button relabels to **Capture Mosaic** so it's clear before pressing it that the whole grid will be shot, and pressing it actually slews to and exposes every pane in pane-major order, not just a single frame; a **Clear Mosaic** button reverts to one. A Session Image block gets its own, independent **Framing…** entry point too, sharing the same underlying logic (not yet built — see `TODO.md`). Constellation/grid overlay isn't drawn yet.
...
- **Still placeholder UI**: Variable Stars, and every Options page except Library, Star Atlas and Planning currently show a "not implemented yet" stub — the domain-core logic behind it (`galileo.plugins.vstarget.*`) already exists and is tested, it just isn't wired to a screen yet.
...
**On the GIL:** people considering Python for a desktop imaging app reasonably ask about the Global Interpreter Lock. It matters here, and Galileo handles it based on measurements rather than assumptions. Pure-`numpy` work (stretching, debayering, histograms, FFTs) releases the GIL, so running it in a background thread keeps the UI responsive. SEP star detection and astroalign registration don't release it: profiled on a 26 MP frame, they froze the event loop for 1.7–2.9 s even from a background thread. That work goes to a `ProcessPoolExecutor` (`galileo.core.compute`), which cuts the stall to under 80 ms. See [SDD §2.1/§2.3](docs/SDD.md) for the details.
