...
- **Responsibility:** Live frame display, histogram, auto-stretch preview, optional debayering of the preview (`IMG-110`, pattern set per camera on the Camera page), per-frame statistics, star overlay, manual capture, panel layout (portrait/landscape, auto-detected from the frame or chosen by the user — `IMG-120`), a mount nudge pad that drives the Mount page's connected adapter through `move_axis` (`IMG-130`), the per-Pier current object (`galileo.current_object`, set from the Star Atlas selection, announced as a `CurrentObjectChangedEvent`, and read by the frame-naming and Solve code — `IMG-140`), sequential multi-frame capture with a per-run gain and optional registration of each frame into the catalog through `LibraryRegistrar.register_capture` (`IMG-150`), live stacking of a run into a running mean (`galileo.livestack`, `IMG-160`), an Options > Imaging screen for the desired saved-frame sample format (`IMG-170`), and the entry point for the flat-wizard workflow (`CAL-060`) — the UI is hosted here rather than in its own primary-navigation section; the capture logic remains in `galileo.calibration` (Section 4.10).
...
- **Satisfies:** `IMG-010`–`IMG-170`, `CAL-060`.
