...
| Linux (ARM64 / Raspberry Pi 5) | Same AppImage pipeline, built on an ARM64 CI runner; documented as a supported target for the network-split deployment topology (Section 7) and for local-GUI/all-in-one use | `NFR-PORT-010`, Section 7 |
...
**Obsy (reference only, not merged):** [Obsy](https://github.com/gordtulloch/obsy) is a Django/KStars-EKOS web application under CC BY-NC-ND 4.0 (Project Scope Document, Section 11, C6) — architecturally unrelated to Galileo's PySide6 desktop stack and not license-compatible for direct code reuse even setting architecture aside. Its contribution is limited to informing `galileo.planning.sky_atlas` (Section 4.8) and `galileo.planning.framing` (Section 4.9): Obsy's `astroplan`-based rise/transit/set computation and its DSS-cutout thumbnail-fetch pattern are reimplemented from scratch under Galileo's own GPL-3.0 codebase (`SKY-080`), not ported. Obsy's Django scheduling functionality is not reused at all (Project Scope Document, Section 5).
...
- **Key design:** Wraps `alpyca`; UDP discovery per the Alpaca spec for device enumeration; HTTP calls issued through the `asyncio`/`qasync` bridge (Section 2.3) to stay non-blocking.
...
- **Key design:** Catalog (≥10,000 objects) is bundled and loaded into a local SQLite database at install time, so search/filter/chart operate fully offline (`NFR-OFFLINE-010`); altitude/rise-transit-set computation follows the approach proven in Obsy's `targets` app (`astroplan` against a configured observer location/timezone — reimplemented under GPL-3.0, not ported, per ADR-003) rather than a from-scratch ephemeris implementation. A target's DSS-cutout thumbnail (`SKY-080`) is fetched and cached on add-to-target-list, the same pattern Obsy used. Observing-location geocoding (`SKY-090`) is adapted from AstroLlama's `get_latlong_tool.py` (ADR-004, Section 2.7), via Open-Meteo.
- **Libraries:** `astropy.coordinates`, `astroplan` (rise/transit/set), SQLite (bundled catalog), an Open-Meteo API client (geocoding).
- **Satisfies:** `SKY-010`–`SKY-090`, `NFR-OFFLINE-010`, `EXT-130`.
...
- **Satisfies:** `GUIDE-010`–`GUIDE-060`, `EXT-050`.
...
- **Local-sensor vs. internet-advisory trust tiers:** a general principle applied consistently across this module — a **local physical sensor** (a connected weather device, `SAFE-070`'s rain sensor, `SAFE-080`'s all-sky-camera cloud classification) is trusted as an input to automated abort decisions (`SAFE-010`); an **internet API** with no local sensor behind it (`SAFE-050` weather forecast, `SAFE-090` aurora, `SAFE-100` smoke) is advisory-only and structurally cannot publish an abort event, for the same reason `SAFE-050` is walled off. `SAFE-080`'s cloud classification reuses the `mlclouddetect` plugin (Section 4.20; `plugins/mlclouddetect/` in the repo) as its reference implementation — the safety module doesn't embed ML inference itself, it consumes a `SafetyMonitorPort` reading like any other device, whether that reading comes from real hardware or a plugin-provided classifier. The pending/hysteresis counter required by `SAFE-080` (N consecutive consistent readings before a state change) is implemented once in this module and reused by any `SafetyMonitorPort` source that requests debouncing, rather than duplicated per-source.
...
- **Responsibility:** Plugin discovery/load/unload, manifest/version compatibility checking, plugin manager UI, fault isolation.
- **Key design:** Plugins are Python packages discovered via `importlib.metadata` entry points, declaring device-port or `SequencerNode` implementations plus a manifest (name, version, API-compatibility range). Every call from the domain core into plugin-provided code is wrapped at the call boundary in an exception handler that logs and disables the offending plugin instance rather than propagating, satisfying `PLUG-040`'s crash-isolation requirement. **Reference implementation:** `plugins/mlclouddetect/` in the repository is a real, standalone plugin implementing `SafetyMonitorPort` (Section 4.1) via ML cloud-cover classification of all-sky camera imagery (reimplemented clean-room from [mlCloudDetect](https://github.com/gordtulloch/mlCloudDetect)'s documented architecture, GPL-3.0 — Project Scope Document Section 6.7/C8). It is the concrete demonstration of everything this section describes: entry-point discovery, a `PluginContext`-driven constructor, a manifest declaring its API-compatibility range, and exception-boundary fault isolation. It exists and runs independently of the rest of Galileo (which has no implementation yet beyond these design documents), so its own `galileo_plugin_api.py` is a local, clearly-labeled stub of the port/context interfaces described here — swapped for a real import once `galileo.core.devices`/`galileo.plugins` exist as code.

**Design note:** this provides fault isolation, not memory/security sandboxing — a plugin runs in-process with full application privileges; stronger sandboxing (e.g. subprocess-per-plugin) is out of scope for v1 and should be revisited if the plugin catalog admits untrusted third-party code at scale. In-process loading also has a licensing consequence, not just a technical one: under Galileo's GPL-3.0 license (ADR-002), plugins are very likely "combined works" and must themselves be GPL-compatible-licensed to distribute.
- **Satisfies:** `PLUG-010`–`PLUG-050`, `NFR-EXT-010`.
...
- **Responsibility:** Repository scanning/ingestion, SHA-256 dedup, metadata-driven file organization, session linking, master calibration-frame creation and application, SEP-based quality metrics, repository statistics, smart-telescope ingestion (SMB/CIFS, FTP/FTPS), and Google Cloud Storage sync.
...
- **Satisfies:** `LIB-010`–`LIB-140`, `EXT-080`, `EXT-090`.

### 4.24 `galileo.vstarget.planning` — Variable Star Target Planning (merged from VSTarget)

- **Responsibility:** AAVSO target sync/filtering, variable-star list display, observable-only filtering, manual import, observation-plan editor, ACP observing-script generation, Simbad lookup. Presented in the UI as a peer-level section to `galileo.planning.sky_atlas`, not nested beneath it (`VST-090`).
- **Key design:** Reuses `galileo.planning.sky_atlas`'s visibility-computation service (Section 4.8) for the observable-only filter (`VST-030`) rather than duplicating rise/transit/set logic.
- **Libraries:** `requests` (AAVSO Target Tool/VSP API client), `astroquery` (Simbad lookup).
- **Satisfies:** `VST-010`–`VST-080`, `EXT-100`, `EXT-110`.

### 4.25 `galileo.vstarget.analysis` — Variable Star Analysis & Photometry (merged from VSTarget)

- **Responsibility:** Remote-telescope image retrieval, plate-solve orchestration (via `galileo.platesolve`), photometric mean-stacking, aperture photometry against AAVSO VSP comparison stars, AAVSO WebObs report generation, transformation-coefficient calibration, exposure calculator, AAVSO finder-chart generation.
