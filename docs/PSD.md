...
| G9 | Harvest Obsy's target-visibility and sky-survey-thumbnail approach as design/implementation reference for the `SKY`/`FRAME` domains; retire Obsy itself, excluding its scheduling functionality |
...
### 6.4 Obsy Feature Inventory (harvested elements only)

Compiled from [github.com/gordtulloch/obsy](https://github.com/gordtulloch/obsy). Obsy as a whole is retired (Section 2); only the elements below inform Galileo's `SKY`/`FRAME` domains, per the author's guidance that its FITS-management functionality is superseded by AstroFiler and its scheduling was too cursory to carry forward.
...
| `SKY` | Sky Atlas | Deep-sky object catalog with search/filter and altitude/visibility charting; horizon obstruction definitions; visibility-computation approach informed by Obsy (Section 6.4) | MVP |
| `FRAME` | Framing Assistant | Sky-survey image overlay, field-of-view preview against connected camera/telescope geometry, mosaic panel planning, offline star-field rendering; sky-survey thumbnail approach informed by Obsy (Section 6.4) | Phase 2 |
...
- **C6** — Obsy is licensed **CC BY-NC-ND 4.0**, not GPL-3.0. Since the same author holds the copyright, reusing Obsy's own concepts/logic in Galileo is not a license violation, but a Creative Commons NoDerivatives license is a meaningful signal to treat Obsy's contribution as architectural reference to be reimplemented under Galileo's GPL-3.0, not code to vendor or copy verbatim (unlike AstroFiler and VSTarget, which are merged directly). If Obsy ever had contributors other than the author, that should be confirmed before reuse.
- **C7** — AstroLlama is GPL-3.0 licensed, so its selectively harvested tool functions (Section 6.5) carry no license tension, unlike Obsy. Its geocoding/weather tools depend on the Open-Meteo API, which requires no API key for non-commercial-scale use and has no license/data-rights conflict with GPL-3.0 distribution — Galileo only calls the API at runtime, it does not redistribute Open-Meteo's data.
- **C8** — mlCloudDetect is GPL-3.0 licensed (direct reuse, no tension). MCP's repository-level license is unspecified by the author (null on GitHub). Treated the same way as Obsy (C6): same-author reuse is not a legal problem, but code is reimplemented clean-room from documented behavior rather than copied verbatim, and this should be confirmed/formalized (e.g. by adding an explicit license to the MCP repo) before any literal MCP source is vendored rather than reimplemented.
...
- Obsy (retired; harvested for `SKY`/`FRAME` domains): https://github.com/gordtulloch/obsy
