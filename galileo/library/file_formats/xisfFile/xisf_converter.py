...
try:
    import lz4.frame
except ImportError:  # only lz4-compressed XISF blocks need it
    lz4 = None
...
class XISFConverter:
    ...
    def _read_file(self):
        ...
        try:
            with open(self.file_path, 'rb') as f:
                ...
                logger.debug("Binary header size: 16 bytes")
    ...
    def _read_image_data(self) -> np.ndarray:
        ...
        if compression_codec:
            ...
            if compression_codec in ('zlib', 'zlib+sh'):
                ...
                try:
                    if lz4 is None:
                        raise RuntimeError("the 'lz4' package is not installed")
    ...
    def convert_to_fits(self, output_path: str | None = None) -> str:
        """
