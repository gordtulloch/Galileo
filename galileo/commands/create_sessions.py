...
"""
galileo.commands.create_sessions - Command line utility to create sessions for unassigned FITS files
...
Usage:
    python -m galileo.commands.create_sessions [options]
...
Example:
    ...
    python -m galileo.commands.create_sessions
    
    # Create sessions with verbose output
    python -m galileo.commands.create_sessions -v
    
    # Only create light sessions
    python -m galileo.commands.create_sessions -l
    
    # Only create calibration sessions
    python -m galileo.commands.create_sessions -C
    
    # Regenerate all sessions (clear existing first)
    python -m galileo.commands.create_sessions -r
    
    # Only create sessions for unassigned files
    python -m galileo.commands.create_sessions -n
    
    # Update quality metrics for all existing sessions
    python -m galileo.commands.create_sessions -q
"""
...
def setup_logging(verbose=False):
    ...
    # Configure logging to both file and console - using central library.log
    logging.basicConfig(
        level=level,
        format=format_str,
        handlers=[
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python -m galileo.commands.create_sessions                # Create both types of sessions
    python -m galileo.commands.create_sessions -v            # Verbose output
    python -m galileo.commands.create_sessions -l            # Only light sessions
    python -m galileo.commands.create_sessions -C            # Only calibration sessions
    python -m galileo.commands.create_sessions -r            # Clear all sessions and regenerate
    python -m galileo.commands.create_sessions -n            # Only create sessions for unassigned files
    python -m galileo.commands.create_sessions -q            # Update quality metrics for existing sessions
        """
    )
    ...
    try:
        logger.info("=== Galileo Session Creator Starting ===")
        ...
        if args.update_quality:
            ...
            logger.info("=== Quality Metrics Update Complete ===")
        ...
        if args.regenerate:
            ...
            logger.info("All existing sessions were cleared and recreated")
        elif args.new_only:
            logger.info("=== New-Only Session Creation Complete ===")
            logger.info("Sessions created only for files without existing sessions")
