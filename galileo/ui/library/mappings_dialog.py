...
from galileo.library.models import fitsFile as FitsFileModel, Mapping as MappingModel

logger = logging.getLogger(__name__)


class MappingsWidget(QWidget):
    """Mappings page of the Library section: rules that rewrite FITS header values."""

    # Emitted after mappings are saved and applied, so views of the catalog can refresh.
    mappings_applied = Signal()
    ...
    def init_ui(self):
        ...
        button_box.button(QDialogButtonBox.Save).clicked.connect(self.accept_mappings)
        button_box.button(QDialogButtonBox.Reset).clicked.connect(self.revert_mappings)
