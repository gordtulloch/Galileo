...
class ImagesWidget(QWidget):
    ...
    def _add_variable_star(self, object_name: str) -> None:
        ...
        try:
            from galileo.library.models import VariableStars
    ...
    def _regenerate_sessions(self, trigger):
        ...
        if sessions_widget is None:
            logger.warning("No sessions widget attached; skipping auto-regeneration after %s", trigger)
            return
        try:
            logger.info("Auto-regenerating sessions after %s...", trigger)
            sessions_widget.auto_regenerate_sessions()
        except Exception as e:
            logger.error(f"Error auto-regenerating sessions after {trigger}: {e}")
