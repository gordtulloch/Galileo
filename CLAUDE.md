...
Galileo is a cross-platform (Windows/macOS/Linux, including Raspberry Pi 5-class ARM) astrophotography imaging application, built on INDI and ASCOM Alpaca device protocols rather than Windows-only ASCOM/COM drivers. It consolidates three of the author's prior projects — AstroFiler (image library/calibration, merged directly into core) and Obsy (sky-atlas/framing logic, ported directly into core) — into one PySide6 desktop app, plus VSTarget (AAVSO variable-star planning/photometry), which ships as a first-party plugin against Galileo's own plugin architecture rather than a core-embedded merge.

**Note on repo state:** `README.md`'s Status section and `CHANGELOG.md` are kept up to date as of each change (see below) — check both for what's actually implemented versus still a placeholder. The design docs in `docs/` are the authoritative source for intended architecture, which is not the same thing as current implementation status. **Plugin documentation lives in its own parallel chain**: `docs/plugins/<plugin-name>/{PSD,SRS,SDD,RTM}.md`, structurally mirroring the core `docs/` chain at plugin scope — e.g. `docs/plugins/vstarget/`. A plugin's own requirements (`VST`/`VST-AN` for VSTarget) are not in the core SRS/SDD/RTM; they're in its own document chain, consuming core requirements (`PLUG`, and others) by reference.
...
**Develop and test on Python 3.11** (`.python-version`; the project `.venv` is 3.11) — the lowest supported version, because Raspberry Pi OS Bookworm ships 3.11. Developing on a newer Python lets 3.12+-only stdlib calls slip in unnoticed. `requires-python` stays `>=3.11` (not capped — Pi OS Trixie is 3.13); ruff's `target-version` and mypy's `python_version` are both pinned to 3.11 and move together with it when the floor is raised.

## Test architecture

Tests are organized **by SRS requirement domain, one file per domain**, not by source module — e.g. `tests/test_arch.py` covers `ARCH-*` requirements. `SEQ`/`SEQ-ADV` requirements are retired and renumbered `SES-*` (`docs/SRS.md` Sections 4.5/4.5a/4.6); `tests/test_ses.py` covers the whole `SES` domain (execution engine, Sessions-screen authoring/lifecycle, and nested instruction/condition/trigger blocks) in one file, replacing the former `test_seq.py`/`test_seq_adv.py`. `VST-AN-*` requirements moved to the VSTarget plugin's own SRS (`docs/plugins/vstarget/SRS.md`); a `test_vst_an.py` file covers that plugin's requirements, not a core domain. The mapping from short test-file suffix to domain is in each file's module docstring (e.g. `test_eqp.py` → `EQP`, `test_foc.py` → `FOC`/Autofocus, `test_mflip.py` → `MFLIP`/Meridian Flip, `test_nfr.py` → cross-cutting non-functional requirements).
...
- **Plugins** register new adapters or new domain-core extensions (`SequencerNode` implementations) against the exact same port interfaces used internally — there is no separate "plugin API." `galileo.plugins.vstarget.planning`/`galileo.plugins.vstarget.analysis` are themselves shipped as first-party pre-loaded plugins (the reference implementation of this boundary), not special-cased core modules — and, correspondingly, their own requirements/design are specified in `docs/plugins/vstarget/`, not embedded in core `docs/SRS.md`/`SDD.md`.
...
**Concurrency model** (SDD §2.3): the Qt UI thread owns the event loop and never blocks; async I/O (Alpaca HTTP, external solver/guider processes) runs via `asyncio` bridged through `qasync`; INDI I/O runs on a per-server reader thread in `galileo.adapters.indi_client` (a native INDI-protocol client — `pyindi-client` is not used because libindi can't be built on Windows), called via `asyncio.to_thread` so it works from any event loop; CPU-bound work that **holds the GIL** (SEP star detection, astroalign registration) goes to the shared `ProcessPoolExecutor` in `galileo.core.compute` via `await run_cpu(fn, *args)` (`run_cpu_sync` from a `QThread`, `imap_cpu` for an ordered, memory-bounded batch); in a thread it would still stall the UI for seconds. Pure-numpy work releases the GIL and uses `asyncio.to_thread` instead, which avoids copying frames between processes. Functions sent to the pool must be module-level and picklable. The test suite sets `GALILEO_CPU_WORKERS=0` (autouse fixture) so monkeypatched functions still take effect; only `test_nfr.py`'s TC-NFR-PERF-020 tests use the real pool.
...
**Star detection uses SEP everywhere in core** (autofocus, imaging-tab stats, library quality metrics) — `photutils` is deliberately reintroduced only in the VSTarget plugin's `galileo.plugins.vstarget.analysis` for aperture photometry, a different problem SEP doesn't solve. Don't conflate the two or "unify" them; `docs/plugins/vstarget/SDD.md` §2.1 explains why they coexist (core SDD §2.6 is a pointer to that section, not the explanation itself, since this decision moved with the plugin's own docs).
