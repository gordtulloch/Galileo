...
**Concurrency model** (SDD §2.3): the Qt UI thread owns the event loop and never blocks; async I/O (Alpaca HTTP, external solver/guider processes) runs via `asyncio` bridged through `qasync`; INDI I/O runs on `pyindi-client`'s own connection thread, marshaled back via queued Qt signals; CPU-bound work (star detection, HFR curve fitting) is dispatched to a `ProcessPoolExecutor`, never run on the UI thread.
