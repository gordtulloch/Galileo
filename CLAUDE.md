...
Tests are organized **by SRS requirement domain, one file per domain**, not by source module — e.g. `tests/test_arch.py` covers `ARCH-*` requirements. `SEQ`/`SEQ-ADV` requirements are retired and renumbered `SES-*` (`docs/SRS.md` Sections 4.5/4.5a/4.6); a test file still named `test_seq_adv.py` predates that rename and should be read as covering the `SES-3xx` nested-block range, not literal `SEQ-ADV-*` IDs, until it's renamed to match. `VST-AN-*` requirements moved to the VSTarget plugin's own SRS (`docs/plugins/vstarget/SRS.md`); a `test_vst_an.py` file covers that plugin's requirements, not a core domain. The mapping from short test-file suffix to domain is in each file's module docstring (e.g. `test_eqp.py` → `EQP`, `test_foc.py` → `FOC`/Autofocus, `test_mflip.py` → `MFLIP`/Meridian Flip, `test_nfr.py` → cross-cutting non-functional requirements).
...
| `galileo.vstarget.planning` / `.analysis` | Vendored VSTarget: AAVSO planning and photometry, shipped as first-party plugins — requirements/design in `docs/plugins/vstarget/`, not core `docs/` |
